...
inline static uint64_t Hacl_Impl_SHA3_rotl(uint64_t a, uint32_t b)
{
...
static void Hacl_Impl_SHA3_storeState(uint32_t rateInBytes, uint64_t *s, uint8_t *res)
{
  uint8_t b[200U] = { 0U };
  for (uint32_t i = (uint32_t)0U; i < (uint32_t)25U; i = i + (uint32_t)1U)
  {
...
)
{
  ...
  Hacl_SHA3_sha3_224(msg_len, msg_, test224);
  Hacl_SHA3_sha3_256(msg_len, msg_, test256);
  Hacl_SHA3_sha3_384(msg_len, msg_, test384);
  Hacl_SHA3_sha3_512(msg_len, msg_, test512);
...
static void test_shake128(uint32_t msg_len, uint8_t *msg, uint32_t out_len, uint8_t *expected)
{
  ...
  Hacl_SHA3_shake128_hacl(msg_len, msg_, out_len, test);
...
static void test_shake256(uint32_t msg_len, uint8_t *msg, uint32_t out_len, uint8_t *expected)
{
  ...
  uint8_t test[out_len];
  memset(test, 0U, out_len * sizeof test[0U]);
  Hacl_SHA3_shake256_hacl(msg_len, msg_, out_len, test);
  if (!Lib_PrintBuffer_result_compare_display(out_len, test, expected))
  {
    exit((int32_t)255);
  }
